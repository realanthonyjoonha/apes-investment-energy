...
**These tools are mandatory for all candidate validation, price pulls, and historical bars.** Web search is only for news, analyst commentary, and discovery — never for prices. If you find yourself thinking "MMD seems unavailable," call the tool anyway — it IS there. The tool has been verified working in this exact session configuration.
